...
public class BotBeh : MonoBehaviour {
	...
	private uint minerals = 0;
	
	public Texture2D mineralsIconTexture;
	...
	{
		if (collider.gameObject.CompareTag("Minerals"))
			CollectMinerals(collider);
		//else
			//HitByLaser(collider);
	}
	...
	void CollectMinerals(Collider2D mineralCollider)
	{
		minerals++;
		
		Destroy(mineralCollider.gameObject);
	...
	void OnGUI()
	{
		DisplayMineralsCount();
		
	//	DisplayRestartButton();
	}
	
	void DisplayMineralsCount()
	{
		Rect mineralIconRect = new Rect(10, 10, 32, 32);
		GUI.DrawTexture(mineralIconRect, mineralsIconTexture);                         
		...
		Rect labelRect = new Rect(mineralIconRect.xMax, mineralIconRect.y, 60, 32);
		GUI.Label(labelRect, minerals.ToString(), style);
	}
